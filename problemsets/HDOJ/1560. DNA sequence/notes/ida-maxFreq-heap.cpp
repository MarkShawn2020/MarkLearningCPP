...
struct State {
    ...
    int pos[N_SEQS];
    int flag[4][8];
    priority_queue<pair<int, int>> heap[4]; // id, freq, seq
    int getSurplus() {
        for(int id=0; id<4; ++id)
            while (flag[id][heap[id].top().second])
                --flag[id][heap[id].top().second],
                heap[id].push({heap[id].top().first-1, heap[id].top().second}),
                heap[id].pop();
        return max(max(heap[0].top().first, heap[1].top().first),
                   max(heap[2].top().first, heap[3].top().first));
    }
}state;
