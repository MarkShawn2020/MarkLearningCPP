...
const int weights[9] = {
        0, 97, 89, 83, 79, 73, 71, 67, 61
//        0, 37, 31, 29, 23, 19, 17, 13, 11
};
int memory[4001];
        ...
        for(int id=1; id<=4; id++)
        {
            ...
            for(int i=0; i<nSeqs; i++)
            {
                ...
                for(int j=pos[i]; j<lens[i]; j++)
                    if(seqs[i][j] == id)
                        ++freq_id_of_iSeq;
                if(freq_id_of_iSeq > freq_id)
                    freq_id = freq_id_of_iSeq;
            }
            ans += freq_id;
        }
        return memory[hash] = ans;
    }
}state;
...
int main()
{
    ...
    auto e = clock();
    printf("time: %.4f ms, kHash: %d, kNHash: %d", double (e-s)/CLOCKS_PER_SEC*1000, kHash, kNHash);
    return 0;
}
